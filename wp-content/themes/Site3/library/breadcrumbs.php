...
function theme_breadcrumbs_items($args) {
    ...
    if (!is_front_page()) {
        $items[] = theme_breadcrumbs_link($args, get_home_url(), '', __('Home', 'site3'));
    }
    ...
    if (is_tag()) {
        $items[] = theme_breadcrumbs_text($args, __("Tag: ", 'site3') . single_tag_title('', FALSE));
    }
    if (is_404()) {
        $items[] = theme_breadcrumbs_text($args, __("404 - Page not Found", 'site3'));
    }
    if (is_search()) {
        $items[] = theme_breadcrumbs_text($args, __("Search", 'site3'));
    }
    if (is_year()) {
        $items[] = theme_breadcrumbs_text($args, get_the_time('Y'));
    }
    if (is_author()) {
        $items[] = theme_breadcrumbs_text($args, sprintf(esc_attr(__('View all posts by %s', 'site3')), get_the_author()));
    }
