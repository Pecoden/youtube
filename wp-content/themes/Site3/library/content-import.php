...
class ThemeContentImport {
    ...
    public static function contentImportNoticeAction() {
        ...
        <div id="content-import-notice" class="updated">
            <p>
                <?php echo __('Do you want to import Content?', 'site3'); ?>
                &nbsp; &nbsp; &nbsp; &nbsp;
                <a id="import-import-content" class="import-button button" href="<?php echo $import_href; ?>"><?php echo __('Import content', 'site3'); ?></a>
                <a id="import-replace-content" class="<?php if (get_option('np_imported_content') === false) echo 'hidden'; ?> import-button button" href="<?php echo $import_href; ?>"><?php echo __('Replace previously imported content', 'site3'); ?></a>
                <a id="import-hide-notice" class="import-button button" href="#"><?php echo __('Hide notice', 'site3'); ?></a>
            </p>
        </div>
        <style>
            .import-button {
        ...
        <script>
            jQuery(document).ready(function ($) {
                ...
                function bindImportAction(action, btn) {
                    ...
                    var failMsg = <?php echo json_encode(__('An error occurred while importing.', 'site3')); ?>;
                    var captchaKeys;
                    btn.unbind("click").click(function() {
                        ...
                        doAjax(action).done(function (response) {
                            ...
                            if (import_content_can) {
                                successMsg = <?php echo json_encode(__('Content was successfully imported.', 'site3')); ?>;
                                $('#content-import-notice').html('<p>' + successMsg + '</p>' + captchaMsg);
                            } else {
                                successMsg = <?php echo json_encode(__('Please install and activate <a href="themes.php?page=tgmpa-install-plugins&plugin_status=install">Nicepage plugin</a> for import content.', 'site3')); ?>;
    ...
    public static function pluginRequiredNoticeAction() {
        ...
        <div id="content-import-notice" class="notice notice-warning">
            <p><?php _e('Import process required plugin to be active.', 'site3'); ?></p>
            <p><?php $tgmpa->is_plugin_installed('nicepage')
                    ? _e('Please activate plugin to continue.', 'site3')
                    : _e('Please install and activate plugin to continue.', 'site3'); ?></p>
        </div>
        <?php
    }
