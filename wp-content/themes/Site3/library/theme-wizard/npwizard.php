...
class Npwizard {
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'verify_text'    => esc_html('verifying', 'site3'),
    ...
    public function menu_page() {
        add_theme_page(esc_html__('Theme Wizard', 'site3'), esc_html__('Theme Wizard', 'site3'), 'manage_options', $this->page_slug, array($this, 'wizard_page'));
    }
    ...
    public function wizard_page() {
        ...
        <div class="wrap npwizard-wrap-perent">
            ...
            if (defined('APP_PLUGIN_VERSION') && isset($GLOBALS['npThemeVersion']) && (float)APP_PLUGIN_VERSION > (float)$GLOBALS['npThemeVersion']) {
                ...
                echo sprintf('<div class="npwizard-warning"><p>%s</p></div>', 'The active theme has a version lower than the plugin version. Please update the theme too.', 'site3');
            }
            ?>
        </div><!-- .npwizard-wrap -->
    ...
    public function get_steps() {
        $steps = array(
            'done' => array(
                'id'			=> 'done',
                'title'			=> __('Your website is ready!', 'site3'),
                'icon'			=> 'yes',
                'view'			=> 'get_step_done',
                'callback'		=> ''
            )
        ...
        $import_content_step = array(
            'content' => array(
                'id'			=> 'content',
                'title'			=> __('Import Content', 'site3'),
                'icon'			=> 'welcome-content-menus',
                'view'			=> 'get_step_content',
                'callback'		=> 'import_content',
                'callback2'		=> 'replace_content',
                'button_text'	=> __('Import Content', 'site3'),
                'button2_text'	=> __('Replace previously imported Content', 'site3'),
                'can_skip'		=> true,
                'can_replace'	=> true
            ),
        );
        ...
        $install_plugin_step = array(
            'plugins' => array(
                'id'			=> 'plugins',
                'title'			=> __('Install Theme Plugins', 'site3'),
                'icon'			=> 'admin-plugins',
                'view'			=> 'get_step_plugins',
                'callback'		=> 'install_plugins',
                'button_text'	=> __('Install Plugins', 'site3'),
                'can_skip'		=> false
            ),
        );
    ...
    public function get_step_plugins() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Install plugins included with the Theme. <br>Click the "Install Plugins" button to start the installation.', 'site3')
        );
    ...
    public function get_step_content() {
        ...
        if($hideImport) {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Content has already been imported. Please skip this step', 'site3')
           );
        } else {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Theme has Pages, Images, Menu, Header, and Footer. </br></br>Do you want to import the Content?', 'site3')
            );
        }
        $content['import_options'] = sprintf(
            ...
            __('Import Sidebars Content', 'site3')
        );
        if (class_exists('Woocommerce')) {
            $content['import_options'] .= sprintf(
                ...
                __('Import Products Content to Woocommerce', 'site3')
            );
        }
    ...
    <div class="theme-appearance-options">
        <label for="np_theme_appearance">' . __('Choose the source for your site\'s Colors, Fonts, Header, and Footer, as well as for the Blog, E-Commerce, and other Templates.', 'site3') . '</label>
        </br></br>
        <select name="np_theme_appearance" id="np_theme_appearance">
            <option value="theme-option" ' . selected(get_option('np_theme_appearance'), 'theme-option', false) . '>' . __('Theme', 'site3') . '</option>
            <option value="plugin-option" ' . selected(get_option('np_theme_appearance'), 'plugin-option', false) . '>' . __('Plugin', 'site3') . '</option>
        </select>
    </div>
    <p>' . sprintf(
                __('You can change it later in the <a href="%s">Nicepage Settings</a>', 'site3'),
    ...
    public function get_step_done() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Congratulations! The theme has been activated and your website is ready.', 'site3')
        );
        $content['summary'] .= sprintf( '<p>%s</p>', 'Create a new page with the Nicepage Editor.', 'site3' );
	...
	public function setup_plugins() {
		if (! check_ajax_referer('npwizard_nonce', 'wpnonce') || empty($_POST['slug'])) {
			wp_send_json_error(array('error' => 1, 'message' => esc_html__('No Slug Found', 'site3')));
		}
		$json = array();
		...
		foreach ($plugins['activate'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Activating Plugin', 'site3'),
		...
		foreach ($plugins['update'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Updating Plugin', 'site3'),
				);
				break;
			}
		}
		foreach ($plugins['install'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Installing Plugin', 'site3'),
				);
				break;
			}
		}
		if ($json) {
			...
			wp_send_json(array('done' => 1, 'message' => esc_html__('Success', 'site3')));
		}
		exit;
	}
    ...
    public function setup_content() {
        ...
        $json = array(
            ...
            'message'       => esc_html__('Import Content', 'site3'),
       );
        if ($json) {
            $json['hash'] = md5(serialize($json)); // used for checking if duplicates happen, move to next plugin
            wp_send_json($json);
